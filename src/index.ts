...
import { z } from "zod";
import { getAllPapers, getPaperByFilter, PaperFilter, upload } from "./helper.js";
...
server.tool(
    "get-all-papers-in-databse",
    "Get all the papers available in the database",
    {},
    async (_args, _extra) => {
...
server.tool(
    "get-all-papers-based-on-filters",
    "Get all the papers based on filters like semester, department, subject name, and exam type",
    {
...
server.tool(
    "upload-paper-to-database",
    "Upload all the details of the paper to the database i.e cloudUrl, semester, department, subject name, and exam type. These details are required to upload the paper",
    {
        semester: z.string(),
        department: z.string(),
